...
If feign client refresh is enabled then each feign client is created with `feign.Request.Options` as a refresh scoped bean. This means properties like `connectTimeout` and `readTimeout` can be refreshed against any feign client instance via `POST /actuator/refresh`.

By default refresh behaviour in feign clients is disabled. Use the following property to enable refresh behaviour.
[source,java]
----
feign.client.refresh-enabled=true
----
TIP: DO NOT annotate `@FeignClient` interface with `@RefreshScope` annotation.
