...
----
When flag is set to true, and oauth2 client context with resource details are present, a bean of class `OAuth2FeignRequestInterceptor` will be created. Since now, before each request, interceptor will resolve required access token and include it as header.
Sometimes when load balancing is enabled for Feign clients, You may want to use load balancing for fetching access token too. To achieve this, You should ensure that load balancer is on classpath (`spring-cloud-starter-loadbalancer`), as well as explicitly enable load balancing for `OAuth2FeignRequestInterceptor` by setting following flag:
----
