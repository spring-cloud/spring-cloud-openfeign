...
* with url wrapped under `org.springframework.cloud.openfeign.RefreshableUrl`. This means url of Feign client if defined
with the property `spring.cloud.openfeign.client.config.{feignName}.url` can be refreshed against any Feign client instance.

The above properties can be refreshed through `POST /actuator/refresh`.
...
URL to a Feign client can be provided in any of the following ways:

|===
|Case |Example |Details

|Url is provided in the `@FeignClient` annotation.
|`@FeignClient(name="testClient", url="http://localhost:8081")`
|Url will be resolved from url attribute of annotation without load balancing.

|Url is provided in the `@FeignClient` annotation as well as in
configuration properties.
|`@FeignClient(name="testClient", url="http://localhost:8081")` and property defined in application.yml as
`spring.cloud.openfeign.client.config.testClient.url=http://localhost:8081`
|Url will be resolved from url attribute of annotation without load balancing.
Url provided in the configuration properties remains unused.

|Url is not provided in the `@FeignClient` annotation but provided in configuration properties.
| `@FeignClient(name="testClient")` and property defined in application.yml as
`spring.cloud.openfeign.client.config.testClient.url=http://localhost:8081`
|Url will be resolved from configuration properties without load balancing. if
`spring.cloud.openfeign.client.refresh-enabled=true` then url defined in configuration properties can be refreshed as described in <<Spring `@RefreshScope` Support,Spring RefreshScope Support>>.

|Url is neither provided in the `@FeignClient` annotation nor in configuration properties.
|`@FeignClient(name="testClient")`
|Url will be resolved from name attribute of annotation with load balancing.
