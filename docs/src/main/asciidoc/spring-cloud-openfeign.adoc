...
Sometimes when load balancing is enabled for Feign clients, You may want to use load balancing for fetching access tokens, too. To do so, you should ensure that the `Spring Cloud LoadBalancer` is on the classpath and explicitly enable load balancing for `OAuth2FeignRequestInterceptor` by setting the following flag:
----
